...
*/
/**************************************************************************/
MAX31865::MAX31865(int8_t spi_cs, SPIClass *spi)
    : _spiConfig{spi, spi_cs, SPISettings(4000000, MSBFIRST, SPI_MODE1)} {}
...
bool MAX31865::begin(max31865_numwires_t wires) {
  pinMode(_spiConfig.cs, OUTPUT);
  digitalWrite(_spiConfig.cs, HIGH);
  _spiConfig.spi->begin();
...
void MAX31865::autoConvert(bool b) {
  _autoMode = b;
  enableBias(_autoMode); // enable bias current for auto mode
  uint8_t t = readRegister8(MAX31865_CONFIG_REG);
  if (_autoMode) {
...
float MAX31865::temperature(float RTDnominal, float refResistor) {
  if (_autoMode) return calculateTemperature(readRTDauto(), RTDnominal, refResistor);
  return calculateTemperature(readRTD1shot(), RTDnominal, refResistor);
}
/**************************************************************************/
/*!
...
*/
/**************************************************************************/
uint16_t MAX31865::readRTD1shot(void) {
  clearFault();
  enableBias(true);
  delay(10);
  uint8_t t = readRegister8(MAX31865_CONFIG_REG);
...
/**************************************************************************/
/*!
    @brief Read the raw 16-bit value from the RTD_REG in auto conversion mode
    @return The calculated temperature
*/
/**************************************************************************/
uint16_t MAX31865::readRTDauto(void) {
  clearFault();
  uint16_t rtd = readRegister16(MAX31865_RTDMSB_REG);
...
void MAX31865::readRegisterN(uint8_t addr, uint8_t buffer[], uint8_t n) {
  #ifdef MAX31865_DEBUG
    Serial.printf("I%d R 0x%02X Q%d\n", deviceNum, addr, n);
    Serial.flush();
  #endif

  addr &= 0x7F; // make sure top bit is not set

  _spiConfig.spi->beginTransaction(_spiConfig.settings);

  digitalWrite(_spiConfig.cs, LOW);
  _spiConfig.spi->transfer(&addr, 1);
  _spiConfig.spi->transfer(buffer, n);
  digitalWrite(_spiConfig.cs, HIGH);

  _spiConfig.spi->endTransaction();
...
void MAX31865::writeRegister8(uint8_t addr, uint8_t data) {
  #ifdef MAX31865_DEBUG
    Serial.printf("I%d W 0x%02X D 0x%02X\n", deviceNum, addr, data);
    Serial.flush();
  #endif
  ...
  digitalWrite(_spiConfig.cs, LOW);
  _spiConfig.spi->transfer(buffer, 2);
  digitalWrite(_spiConfig.cs, HIGH);
  _spiConfig.spi->endTransaction();
