/***************************************************
  ...
  Modified by J Peake - PeakeElectronicInnovation for non-blocking
  reads in auto mode (original library does not seem to natively
  support auto convert). Thanks to Limor Fried/Ladyada for the
  original library.
 ****************************************************/
...
class MAX31865 {
public:
  MAX31865(int8_t spi_cs, SPIClass *spi = &SPI);
  ...
  uint16_t readRTD1shot(void);
  uint16_t readRTDauto(void);
...
private:
  ...
  bool _autoMode = false; // keep track of mode for correct function calls by temperature()
